...
## Этап 14: добивка хвостов (все известные кандидаты закрыты)

### Исправлено (этап 14)
43. **CalendarView.tsx** — весь `props` замораживался в `useMemo(..., [])` перед PropsProvider: смена
    handler/renderItem/слотов родителем после маунта игнорировалась навсегда (та же семья, что №42, уровнем выше).
    Контекст теперь собирается на каждый рендер (`payload` остаётся useSingleton — осознанно стабилен);
    перф не страдает: консюмеры не мемоизированы и ре-рендерились каскадом и раньше.
    Тест: calendarview «props changes after mount reach consumers» (на старом коде падает — проверено).
    Заодно убраны no-op `.set("year", get("year"))` / `.set("month", ...)` в onYearChange/onMonthChange.
44. **CalendarView/Day.tsx** — `isActive` вычислялся в Calendar и передавался, но Day его игнорировал:
    выбранная дата визуально не отмечалась. Добавлен класс buttonActive (alpha primary 0.18).
45. **mvvm/Collection.ts `_change`** — при явной смене id сущности через `setData({id: другой})` карта `_ids`
    протухала: findById по новому id кидал EntityNotFoundError, по старому — отдавал сущность с чужим id.
    Теперь `_change` пересинхронизирует `_ids` (O(1) fast-path, скан только при фактической смене id).
    Тест в mvvm.test.ts (на старом коде падает — проверено). Закрывает заметку этапа 5.
46. **List/hooks/useIntersection.tsx IntersectionProvider** — эффект подписки reloadSubject был без deps-массива
    (переподписка на каждый рендер, cleanup корректный) — deps `[withRestorePos]`. Закрывает заметку этапа 12.
47. **hooks/usePreventAutofill.ts** — handleTouchStart/handleContextMenu имели deps `[onFocus]` вместо
    `[onTouchStart]`/`[onContextMenu]` (копипаста). Закрывает заметку этапа 6.

### Осталось (осознанно НЕ трогаем)
- deepClone поверхностно копирует массивы объектов — поведение functools-kit (upstream).
- Async.disabled «once enabled», глобальный clearSubject в ModalManagerProvider, handleReady в минус — by design.
- chooseMultipleFiles: подвисание промиса при отмене диалога — практически недостижимо (waitForResume-цикл).
- downloadBlob: запросный заголовок Access-Control-Allow-Origin — сетевое поведение, не трогаем без решения.

### Закрыт пробел покрытия (этап 13)
src/styles (ThemeProvider/makeStyles/index — тонкие обёртки tss-react и MUI, чисто), src/polyfills.ts
(window=globalThis для SSR, чисто), typings.d.ts — каталоги, пропущенные всеми этапами. Теперь src — 100%.
