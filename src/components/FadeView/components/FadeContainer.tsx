    ...
    useEffect(() => {
        if (elementRef) {
            ...
            if (scrollViewRef) {
                ...
                const updateBottom = debounce(() => setBottomState({...bottomCopy}), FADE_ANIMATION_DELAY);
                const updateRight = debounce(() => setRightState({...rightCopy}), FADE_ANIMATION_DELAY);
                let lastScrollHeight = -1, lastScrollWidth = -1;
                let lastScrollTop = -1, lastScrollLeft = -1;
                const handleScroll = () => {
