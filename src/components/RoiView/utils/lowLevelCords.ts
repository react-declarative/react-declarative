...
export const lowLevelCords = (cords: ICord[], naturalHeight = 100, naturalWidth = 100) => cords.map(({
    id, type, height, width, top, left, color, label
}) => type === 'rect' ? rect(id, top, left, height, width, color, label)
        : type === 'square' ? square(id, top, left, Math.max(height, width), color, label)
